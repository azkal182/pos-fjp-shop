...
export default function DebtsPage() {
  ...
  const fetchDebts = useCallback(async () => {
    ...
    try {
      ...
      const outstanding = allDebts.reduce((s: number, d: DebtRow) => s + d.remainingAmount, 0)
      ...
      const oldest = allDebts.length > 0
        ? Math.max(...allDebts.map((d: DebtRow) => {
            const days = Math.floor((Date.now() - new Date(d.debtDate).getTime()) / 86400000)
            return days
          }))
        : null
