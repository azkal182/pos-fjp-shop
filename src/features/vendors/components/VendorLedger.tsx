...
export function VendorLedger({ vendorId, refreshKey }: { vendorId: string; refreshKey?: number }) {
  ...
  const totalInvoice = data.entries
    .filter((e) => e.type === "INVOICE")
    .reduce((s, e) => s + Number(e.amount), 0)
  const totalPaymentOut = data.entries
    .filter((e) => e.type === "PAYMENT_OUT")
    .reduce((s, e) => s + Number(e.amount), 0)
  const totalDepositIn = data.entries
    .filter((e) => e.type === "DEPOSIT_IN")
    .reduce((s, e) => s + Number(e.amount), 0)
  const totalDepositUsed = data.entries
    .filter((e) => e.type === "DEPOSIT_OUT")
    .reduce((s, e) => s + Number(e.amount), 0)
      ...
      <CardHeader className="pb-3">
        ...
        <div className="grid grid-cols-2 gap-3 mt-2">
          <div className="rounded-md bg-red-50 dark:bg-red-950/20 border border-red-100 dark:border-red-900/30 px-3 py-2">
            ...
            <CurrencyDisplay amount={totalInvoice} className="text-sm font-semibold text-red-700 dark:text-red-400" />
          </div>
          <div className="rounded-md bg-green-50 dark:bg-green-950/20 border border-green-100 dark:border-green-900/30 px-3 py-2">
            ...
            <CurrencyDisplay amount={totalPaymentOut} className="text-sm font-semibold text-green-700 dark:text-green-400" />
          </div>
          <div className="rounded-md bg-blue-50 dark:bg-blue-950/20 border border-blue-100 dark:border-blue-900/30 px-3 py-2">
            ...
            <CurrencyDisplay amount={totalDepositIn} className="text-sm font-semibold text-blue-700 dark:text-blue-400" />
          </div>
          <div className="rounded-md bg-amber-50 dark:bg-amber-950/20 border border-amber-100 dark:border-amber-900/30 px-3 py-2">
            ...
            <CurrencyDisplay amount={totalDepositUsed} className="text-sm font-semibold text-amber-700 dark:text-amber-400" />
          </div>
        </div>
      </CardHeader>
