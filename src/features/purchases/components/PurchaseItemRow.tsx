...
export function PurchaseItemRow({ index, onRemove, vendorId }: PurchaseItemRowProps) {
  const { register, setValue, watch, formState: { errors } } = useFormContext<CreatePurchaseInput>()
  ...
  // Baca nilai langsung dari watch (bukan useWatch) agar tidak trigger re-render parent
  ...
  async function fetchCatalogPrice(productId: string): Promise<number | null> {
  ...
  async function selectProduct(product: Product) {
    // Set search text dulu sebelum async call agar input langsung terisi
    ...
    // Set harga default dulu (dari produk), lalu update jika ada catalog
    setValue(`items.${index}.buyPrice`, product.buyPrice, { shouldDirty: true, shouldValidate: false })
    setCatalogPrice(null)

    // Fetch catalog price async — update jika ada
    const catalog = await fetchCatalogPrice(product.id)
    if (catalog !== null) {
      setCatalogPrice(catalog)
      setValue(`items.${index}.buyPrice`, catalog, { shouldDirty: true, shouldValidate: false })
    }
  }
  ...
  return (
    <div className="grid grid-cols-[1fr_72px_130px_90px_36px] gap-2 items-start">
      ...
      <div className="relative">
        ...
        {/* Hidden input untuk productId — dikelola via setValue */}
      ...
      {/* Qty — pakai register biasa, tidak ada konflik */}
      <div>
        <Input
          type="number"
          min={1}
      ...
      {/* Harga Beli — pakai register saja, TANPA value prop agar tidak konflik */}
      <div>
        <div className="relative">
